"""Base test class for vulnerabilities functionality."""
...
from purple_mcp.libs.vulnerabilities.models import PageInfo
...
class ToolFunction(Protocol):
    ...
    diverse tool function signatures (get_vulnerability(str), search_vulnerabilities(list[JsonDict], int, str), etc.)
...
class VulnerabilitiesTestBase:
    """Base class with common test utilities for vulnerabilities tests."""
    ...
    async def assert_tool_success(
        ...
        Args:
            ...
            mock_get_client: Mock for _get_vulnerabilities_client
    ...
    def create_mock_with_connection(
        method_name: str,
        connection_type: type[VulnerabilityConnection],
        items: list[VulnerabilityDetail] | None = None,
    ) -> AsyncMock:
        ...
        Example:
            mock_client = self.create_mock_with_connection(
                ...
                VulnerabilityConnection,
                [test_vulnerability1, test_vulnerability2]
            )
        """
        if items:
            ...
            edge_class = {
                "VulnerabilityConnection": VulnerabilityEdge,
                "VulnerabilityNoteConnection": VulnerabilityNoteEdge,
                "VulnerabilityHistoryItemConnection": VulnerabilityHistoryItemEdge,
            }.get(connection_type.__name__, VulnerabilityEdge)
            ...
            # Empty connection
            connection = MockVulnerabilitiesClientBuilder.create_empty_connection(connection_type)

        return MockVulnerabilitiesClientBuilder.create_mock(method_name, connection)
